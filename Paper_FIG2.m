...
for iSt = 1:length(stID)*.6
    ...
    if iSt < 7
        ...
        p1                  = plot(mFIGe, 'Color', [.5 .5 .5], 'LineWidth', 2, 'LineStyle', '-');
        axF.YTick           = [0 1 2];
        axF.YLim            = [.5 2.5];
        ...
        fi1                 = fill([1:600 600:-1:1],[mFIG+semFIG/2 fliplr(mFIG-semFIG/2)], [0 0 0], 'LineStyle','none');
        fi1.FaceAlpha       = .3;
        p1                  = plot(mFIG, 'Color', [0 0 0], 'LineWidth', 2, 'LineStyle', '-');
        axF.YTick           = [0 5 10];
        axF.YLim            = [-1 10];
...
p1              = plot(frfft,P1./ max(P1), 'k', 'LineWidth', 2);
axFFT.XColor    = [0 0 0];
axFFT.YColor    = [0 0 0];
axFFT.XLim      = [0 50];
axFFT.YLim      = [0 .2];
...
axB             = axes('Position',[row(1)+.18 .06 .12 .2]); hold on
...
axBB                = axes('Position',[row(1)+.31 .06 .12 .2]); hold on
